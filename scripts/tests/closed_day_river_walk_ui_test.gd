...
func _initialize() -> void:
	...
	var test_date := _helpers.find_closed_date_with_choice(fixture_game, "go_out", "river_walk")
	_expect(not test_date.is_empty(), "should find a closed day where river walk is offered")
	...
	var river_button := _helpers.find_button_containing(choice_grid, "한강")
	_expect(river_button != null, "river walk should be visible as an actual choice button")
	...
	river_button.emit_signal("pressed")
	await create_timer(0.25).timeout
	var event_layer := _helpers.find_node(scene, "DayEventCgLayer")
	_expect(event_layer != null, "river walk should immediately show event CG layer")
	_expect(_helpers.find_node(scene, "EventCG") != null, "river walk should show event CG")
	_expect(_helpers.find_node(scene, "EventDialogue") != null, "river walk should show event dialogue")
	for _index in 8:
		event_layer.call("_advance")
		await process_frame
	await create_timer(0.25).timeout
	_expect(flow_button.text == "잠들기", "river walk should end with sleep button active")
	_expect(not flow_button.disabled, "sleep button should be enabled after river walk event")

	print("Closed-day river walk UI test passed.")
	finish_test()
