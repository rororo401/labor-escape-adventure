...
SCRIPT_DOC_PATH = ROOT / "docs/gameplay/phase2_new_event_scripts.md"
DAY_EVENTS_PATH = ROOT / "data/game/day_events.json"

SECTION_TO_GROUP = {
    "집에 있기 신규 40개": "stay_home",
    "외출하기 신규 40개": "go_out",
    "평일 사건 신규 35개": "weekday",
    "밤/돌발 신규 35개": "night",
}
...
def main() -> int:
    phase2_events = _parse_phase2_scripts(SCRIPT_DOC_PATH.read_text(encoding="utf-8"))
    day_events = json.loads(DAY_EVENTS_PATH.read_text(encoding="utf-8"))

    errors = _validate_source_events(phase2_events)
    if errors:
        _print_errors(errors)
        return 1

    promoted = [_promote_event(event) for event in phase2_events]
...
def _parse_phase2_scripts(text: str) -> list[dict]:
    events: list[dict] = []
    current_section = ""
...
def _validate_source_events(events: list[dict]) -> list[str]:
    errors: list[str] = []
    if len(events) != 150:
        errors.append(f"phase2 event count: expected 150, got {len(events)}")
    ...
    for duplicate in _duplicates([event.get("id", "") for event in events]):
        errors.append(f"duplicate phase2 id: {duplicate}")
    for duplicate in _duplicates([event.get("cg_path", "") for event in events]):
        errors.append(f"duplicate phase2 cg_path: {duplicate}")
    return errors
...
def _validate_promoted_events(day_events: dict, promoted: list[dict]) -> list[str]:
    ...
    for event in promoted:
        ...
        if existing and existing.get("cg_path", "") != event["cg_path"]:
            errors.append(
                "phase2 id already exists with a different CG path: %s" % event["id"]
            )
        existing_id = current_id_by_cg_path.get(event["cg_path"], "")
        if existing_id and existing_id != event["id"]:
            errors.append(
                "phase2 CG path is already used by another event: %s -> %s"
